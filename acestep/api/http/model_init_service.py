...
def initialize_models_for_request(
    ...
    offload_to_cpu = env_bool("ACESTEP_OFFLOAD_TO_CPU", True) if offload_to_cpu_env is not None else auto_offload
    use_flash_attention = env_bool("ACESTEP_USE_FLASH_ATTENTION", True)
    offload_dit_to_cpu = env_bool("ACESTEP_OFFLOAD_DIT_TO_CPU", True)
    compile_model = env_bool("ACESTEP_COMPILE_MODEL", True)
    ...
    if init_llm:
        ...
        lm_backend = os.getenv("ACESTEP_LM_BACKEND", "pt").strip().lower()
        ...
        lm_offload = env_bool("ACESTEP_LM_OFFLOAD_TO_CPU", True) if lm_offload_env is not None else offload_to_cpu
