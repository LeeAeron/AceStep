    ...
    async def format_input_endpoint(request: Request, authorization: Optional[str] = Header(None)):
        ...
        with llm_lock:
            if not getattr(app.state, "_llm_initialized", False):
                ...
                backend = os.getenv("ACESTEP_LM_BACKEND", "pt").strip().lower()
                ...
                lm_device = os.getenv("ACESTEP_LM_DEVICE", os.getenv("ACESTEP_DEVICE", "cuda"))
                lm_offload = env_bool("ACESTEP_LM_OFFLOAD_TO_CPU", True)
