...
class TestApplyRepaintWaveformSplice(unittest.TestCase):
    ...
    def test_non_repaint_regions_match_src(self):
        ...
        result = apply_repaint_waveform_splice(
            pred, src, [0.05], [0.15], sample_rate=192000, crossfade_duration=0.0,
        )
        start = int(0.05 * 192000)
        end = int(0.15 * 192000)
    ...
    def test_crossfade_produces_intermediate_values(self):
        pred, src = self._make_tensors(samples=9600)
        result = apply_repaint_waveform_splice(
            pred, src, [0.05], [0.15], sample_rate=192000, crossfade_duration=0.005,
        )
        cf_samples = int(0.005 * 192000)
        start = int(0.05 * 192000)
    ...
    def test_full_repaint_returns_pred_unchanged(self):
        ...
        result = apply_repaint_waveform_splice(
            pred, src, [0.0], [0.1], sample_rate=192000, crossfade_duration=0.01,
        )
    ...
    def test_length_mismatch_pred_longer(self):
        ...
        result = apply_repaint_waveform_splice(
            pred, src, [0.02], [0.08], sample_rate=192000, crossfade_duration=0.0,
        )
    ...
    def test_batch_independent_splice(self):
        ...
        result = apply_repaint_waveform_splice(
            pred, src, [0.02, 0.05], [0.08, 0.15], sample_rate=192000,
            crossfade_duration=0.0,
        )
        s0, e0 = int(0.02 * 192000), int(0.08 * 192000)
        s1, e1 = int(0.05 * 192000), int(0.15 * 192000)
