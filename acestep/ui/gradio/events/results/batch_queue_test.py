...
class StoreBatchInQueueTests(unittest.TestCase):
    ...
    def test_store_first_batch(self):
        ...
        result = store_batch_in_queue(
            batch_queue=queue,
            batch_index=0,
            audio_paths=["/tmp/audio1.wav"],
            generation_info="test info",
            seeds="42",
        )
        self.assertIsInstance(result, dict)
        self.assertIn(0, result)
        self.assertEqual(result[0]["audio_paths"], ["/tmp/audio1.wav"])
    ...
    def test_store_preserves_existing_batches(self):
        ...
        result = store_batch_in_queue(
            batch_queue=queue,
            batch_index=1,
            audio_paths=["/tmp/new.wav"],
            generation_info="new info",
            seeds="99",
        )
        self.assertIn(0, result)
        self.assertIn(1, result)
        self.assertEqual(result[0]["audio_paths"], ["/tmp/old.wav"])
        self.assertEqual(result[1]["audio_paths"], ["/tmp/new.wav"])
